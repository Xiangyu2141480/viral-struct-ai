// viz.jsx — Heavy visualizations for the 3 P0 concept upgrades
// Depends on data.jsx + components.jsx
...
const AbstractStructureBand = ({ segments, total, onSegHover, height = 64 }) =>
<div className="sband abstract" style={{ height }}>
    {segments.map((seg, i) => {
    const dur = seg.end !== undefined ? seg.end - seg.start : seg.dur;
...
const ConcreteFilmStrip = ({ segments, total, height = 64 }) =>
<div className="tline concrete" style={{ height }}>
    ...
    return (
      <div
        ...
        style={{ width: `${w}%`, "--role-color": `var(--r-${seg.role})` }}>
...
const SyncRails = ({ segments, total, height = 36 }) => {
  return (
    <div className="sync-rails" style={{ height }}>
      <svg width="100%" height={height} style={{ display: "block", overflow: "visible" }} preserveAspectRatio="none">
        {segments.map((seg, i) => {
          ...
          const x0 = seg.start / total * 100;
          const x1 = seg.end / total * 100;
          const cx = (seg.start + dur / 2) / total * 100;
          return (
            <g key={seg.id}>
...
const MigrationFlow = () => {
  ...
  const matCy = (i) => matStart + i * (MAT_H + MAT_GAP) + MAT_H / 2;
  const slotCy = (i) => slotStart + i * (SLOT_H + SLOT_GAP) + SLOT_H / 2;
  ...
  const fillByState = { filled: 100, weakly: 60, missing: 30, critical: 15 };
...
const DiagnosticRadar = ({ size = 360, selected, onSelect }) => {
  const segs = SOURCE_VIDEO.segments;
  ...
  const angleFor = (i) => -Math.PI / 2 + Math.PI * 2 * i / n;
  const pointAt = (i, r) => [cx + Math.cos(angleFor(i)) * r, cy + Math.sin(angleFor(i)) * r];

  const fillByState = { filled: 1.0, weakly: 0.6, missing: 0.3, critical: 0.15 };
  const stateColor = {
    filled: "var(--st-filled)",
    weakly: "var(--st-weakly)",
    missing: "var(--st-missing)",
...
const GapHeatmap = ({ selected, onSelect }) => {
  const segs = SOURCE_VIDEO.segments;
  const cellColor = (v) => {
...
/* ============================================================
   ...
   不是独立 Slot —— 坐在共享时间轴的接缝上，按真实时长straddle 边界。
   硬切=细虚线节点（无过渡素材）；叠化/推镜/卡点=实节点 + 时长带。
   risk 复用四态色把"转场撑不撑得起"接回诊断。
   ============================================================ */

const TransitionGlyph = ({ type, color = "var(--text-2)", size = 16 }) => {
  switch (type) {
    case "硬切":
      return (
...
const TransitionSeams = ({ segments, transitions, total, height = 46, showHardCuts = true, selected, onSelect }) => {
  ...
  return (
    <div className="tseam-track" style={{ height }}>
      ...
      {transitions.map((tr, i) => {
        ...
        const meta = TRANSITION_TYPES[tr.applied] || {};

// screens-cd.jsx — Screens 4–6 (the heavy hitters)
// Depends on data.jsx + components.jsx loaded first.

/* ============================================================
   屏 4 · 素材缺口诊断  ★ 重点屏 (the moat)
   ============================================================ */

const STATE_ORDER = ["filled", "weakly", "missing", "critical"];
const STATE_LABELS = { filled: "已满足", weakly: "弱满足", missing: "缺失", critical: "关键缺失" };

/* ── Gap-fill studio: 3 ways to补素材 — 补拍 / HyperFrames / AIGC ── */
const FILL_METHODS = [
...
const GapFillStudio = ({ seg, d, previewActive, onPreview, onToast }) => {
  const fill = d.fill || {};
  const recommended = d.strategy === "aigc" ? "aigc" : (d.strategy === "hyperframes" ? "hyperframes" : "reshoot");
  const [method, setMethod] = useState(recommended);
  const [hfState, setHfState] = useState("idle");      // idle | generating | done
  const [reshootFile, setReshootFile] = useState(null);
  const [aigcFile, setAigcFile] = useState(null);
  const [copied, setCopied] = useState(false);
  const reshootInput = React.useRef(null);
  const aigcInput = React.useRef(null);

  const m = fill[method] || {};

  const copyPrompt = () => {
    try { navigator.clipboard && navigator.clipboard.writeText(m.prompt || ""); } catch (e) {}
    setCopied(true);
    onToast && onToast("Prompt 已复制 · 粘贴到你的 AI 工具");
    setTimeout(() => setCopied(false), 1600);
  };

  const UploadedChip = ({ name, onRe }) => (
    <div style={{
  ...
  return (
    <div style={{ border: "1px solid var(--accent-line)", borderLeft: "3px solid var(--accent)", borderRadius: 5, background: "var(--bg-2)", overflow: "hidden" }}>
      ...
      <div style={{ padding: "0 12px 12px", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {method === "reshoot" && (
          <>
            <div style={{ fontSize: 11.5, color: "var(--text-dim)", lineHeight: 1.5 }}>{m.guide}</div>
            ...
            <input ref={reshootInput} type="file" accept="video/*,image/*" style={{ display: "none" }}
              onChange={e => { if (e.target.files[0]) { setReshootFile(e.target.files[0].name); onToast && onToast("补拍素材已上传 · 待编入时间线"); } e.target.value = ""; }} />
            {reshootFile
              ? <UploadedChip name={reshootFile} onRe={() => setReshootFile(null)} />
              : <button className="btn primary" style={{ justifyContent: "center", padding: "8px 12px" }} onClick={() => reshootInput.current && reshootInput.current.click()}>
        ...
        {method === "hyperframes" && (
          <>
            <div style={{ fontSize: 11.5, color: "var(--text-dim)", lineHeight: 1.5 }}>{m.desc}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span className="eyebrow">将复用</span>
              <div style={{ display: "flex", gap: 5 }}>
                {(m.uses || []).map(u => <span key={u} className="need-chip have">{u.toUpperCase()}</span>)}
              </div>
            </div>
            {hfState === "done" ? (
              <div style={{
              ...
              <button className="btn primary" style={{ justifyContent: "center", padding: "8px 12px" }}
                ...
                onClick={() => { setHfState("generating"); setTimeout(() => { setHfState("done"); onToast && onToast("HyperFrames 已生成补全片段"); }, 1500); }}>
        ...
        {method === "aigc" && (
          <>
            ...
            <div style={{ position: "relative" }}>
              <textarea readOnly value={m.prompt || ""} rows={3}
                style={{
            ...
            <input ref={aigcInput} type="file" accept="video/*" style={{ display: "none" }}
              onChange={e => { if (e.target.files[0]) { setAigcFile(e.target.files[0].name); onToast && onToast("AI 生成视频已上传 · 待编入时间线"); } e.target.value = ""; }} />
            {aigcFile
              ? <UploadedChip name={aigcFile} onRe={() => setAigcFile(null)} />
              : <button className="btn primary" style={{ justifyContent: "center", padding: "8px 12px" }} onClick={() => aigcInput.current && aigcInput.current.click()}>
...
const TransitionFillStudio = ({ tr, onToast }) => {
  const fromSeg = SOURCE_VIDEO.segments.find(s => s.id === tr.from);
  const toSeg   = SOURCE_VIDEO.segments.find(s => s.id === tr.to);
  const fromD = SLOT_DIAGNOSIS[tr.from] || {};
  const toD   = SLOT_DIAGNOSIS[tr.to] || {};
  const frameAvail = (st) => st === "filled" || st === "weakly";
  const fromOK = frameAvail(fromD.state);
  const toOK   = frameAvail(toD.state);
  const aigcReady = fromOK && toOK;
  const recommended = !tr.upgradable ? "cut" : (aigcReady ? "aigc" : "frame");
  // HyperFrames effect the Agent will compose, inferred from the source transition intent
  const HF_EFFECT = { 叠化: "交叉叠化", 推镜: "Ken Burns 推近", 卡点: "节奏闪切", 硬切: "短暂叠化柔化" };
  ...
  const runGen = (msg) => {
    ...
    setTimeout(() => { setGen(prev => ({ ...prev, [method]: "done" })); onToast && onToast(msg); }, 1400);
  };

  const DoneChip = ({ text, onRe }) => (
    <div style={{
      display: "flex", alignItems: "center", gap: 8, padding: "8px 10px", borderRadius: 5,
      background: "var(--st-filled-bg)", border: "1px solid var(--st-filled-line)",
      fontSize: 11.5, color: "var(--st-filled)",
      ...
      <button className="btn" style={{ padding: "2px 8px", fontSize: 10.5 }} onClick={onRe}>重做</button>
    </div>
  );

  const MiniFrame = ({ seg, ok, label }) => (
      ...
      <div style={{ padding: "0 12px 12px", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {method === "aigc" && (
          <>
            ...
            {st === "done"
              ? <DoneChip text="AIGC 补间帧已生成 · 已编入接缝" onRe={() => setGen(p => ({ ...p, aigc: "idle" }))} />
              : <button className="btn primary" style={{ justifyContent: "center", padding: "8px 12px" }}
...
const ScreenDiagnose = ({ onNext, onBack }) => {
  ...
  const summary = STATE_ORDER.reduce((acc, st) => {
        ...
        <div className="panel-body">
          ...
          <div style={{ marginTop: 12, marginBottom: 4, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <span className="mono" style={{ fontSize: 9.5, color: "var(--text-mute)" }}>
              {v.transitions.filter(t => t.state === "filled").length} 已满足 · {v.transitions.filter(t => t.state === "weakly").length} 弱满足 · 0 缺失
            </span>
          </div>
          ...
          <div style={{ display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
            <StateBadge state="filled"   /> <span className="mono dim" style={{ fontSize: 11 }}>{summary.filled} 内容 · {v.transitions.filter(t => t.state === "filled").length} 转场</span>
            <StateBadge state="weakly"   /> <span className="mono dim" style={{ fontSize: 11 }}>{summary.weakly} 内容 · {v.transitions.filter(t => t.state === "weakly").length} 转场</span>
              ...
              <div className="panel slot-detail-panel">
                ...
                <div className="panel-body" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  {tr.state === "filled" ? (
                    ...
                    <TransitionFillStudio key={tr.id} tr={tr} onToast={showToast} />
                  )}
                </div>
              </div>
            );
...
Object.assign(window, { ScreenDiagnose, ScreenCompile });

...
const ScreenSource = ({ onNext }) => {
  const v = SOURCE_VIDEO;
  const T = v.duration;
  const [hoveredSeg, setHoveredSeg] = useState(v.segments[0]);
    ...
    <div className="screen">
      ...
      <div className="panel" style={{ marginBottom: 20 }}>
        ...
        <div className="panel-body">
          ...
          {/* transition seam layer — sits on the shared axis between abstract & concrete */}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 2 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{
                fontFamily: "var(--ff-mono)", fontSize: 10,
                padding: "2px 7px", background: "var(--surface-3)", color: "var(--text-2)",
                border: "1px solid var(--border-hi)",
                fontWeight: 500, letterSpacing: 0.06, borderRadius: 3,
              }}>◇ 转场层</span>
              <span className="eyebrow">特殊 Slot · 富转场=已满足 / 硬切=弱满足下限</span>
            </div>
...
const ScreenMaterials = ({ onNext, onBack }) => {
  const v = SOURCE_VIDEO;
  const T = v.duration;
  ...
  const [toastMsg, setToastMsg] = useState("");
  const [toastVisible, setToastVisible] = useState(false);
  const [productInfo, setProductInfo] = useState({ ...TARGET_PRODUCT });
        ...
        <div className="panel-body" style={{
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <button className="btn" style={{ flex: 1, justifyContent: "center" }} onClick={() => { const input = document.createElement('input'); input.type = 'file'; input.accept = 'image/*'; input.onchange = (e) => { if (e.target.files.length) showToast('商品图已替换: ' + e.target.files[0].name); }; input.click(); }}>
...
Object.assign(window, { ScreenSource, ScreenMaterials });

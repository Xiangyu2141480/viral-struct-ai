...
// (Loaded via Babel; depends on data.jsx being loaded first.)
...
const Spine = ({ activeStep, setStep, labOpen, setLabOpen, projectId }) => {
...
const Sidebar = ({ activeStep, setStep, labOpen, setLabOpen, toolView, setToolView }) => {
  const activeIdx = STEPS.findIndex(s => s.id === activeStep);
  const isToolActive = (name) => toolView === name;
  return (
...
const StructureBand = ({ segments, total, height = 38, thin = false, showLabels = true, onSegHover }) => {
  return (
...
const TimeRuler = ({ duration, intervals = 6 }) => {
  const ticks = [];
...
const TimelineTrack = ({ segments, total, frames = true, height = 72 }) => {
  return (
...
const FramePlaceholder = ({ role, label }) => {
  const drawByRole = {
    hook:    <SvgHookShape />,
    pain:    <SvgPainShape />,
...
const StrategyTag = ({ kind, withIcon = true, size = "md" }) => {
...
const SatisfactionRing = ({ pct = 0, size = 140, label = "整体满足度" }) => {
...
const DropZone = ({ accept = "*", onFiles, label = "拖拽文件到此处，或点击选择", multiple = true }) => {
  ...
  return (
    ...
    >
      <input
        ...
        multiple={multiple}
        style={{ display: "none" }}
        onChange={e => { if (e.target.files.length && onFiles) onFiles([...e.target.files]); e.target.value = ""; }}
      />
...
Object.assign(window, {
  Icon, Spine, Sidebar, StructureBand, TimeRuler, TimelineTrack, FramePlaceholder,
  MatThumb, StateBadge, RoleLegend, ScreenFooter, StrategyTag, STRATEGIES, SatisfactionRing,
  SvgHookShape, SvgPainShape, SvgEmotionShape, SvgProductShape, SvgCompareShape, SvgSocialShape, SvgCtaShape,
  Modal, Toast, DropZone,
});

// data.ts — Mock fixtures for the 爆款结构迁移引擎 prototype
// (Ported from data.jsx; window globals replaced with ES exports.)
...
export const SOURCE_VIDEO: SourceVideo = {
  id: 'douyin_2786341',
...
export const TARGET_PRODUCT: TargetProduct = {
  name: '和田玉福镯 · 送母亲',
  category: '首饰 / 玉石',
  price: '¥ 599',
  stock: 4200,
...
export const TARGET_MATERIALS: Material[] = [
...
//
// The legacy synthesized fields (reshoot.guide/shots, hyperframes.uses/desc,
// aigc.prompt) are KEPT so existing screen code keeps compiling. The new
// structured fields (mirrored from the server-side DiagnosisFill in
// apps/api/src/services/structAdapter/structTypes.ts) are ADDITIVE/optional and
// carry the FULL Director-authored option payload for the real reshoot brief /
// HyperFrames card / AIGC job-card.
...
export interface Diagnosis {
  ...
  fill?: DiagnosisFill;
  /** The Director Agent's recommended resolution channel for this slot. */
...
export const SLOT_DIAGNOSIS: Record<string, Diagnosis> = {
  s1: {
    state: 'missing',
    have: [],
    need: ['主体特写', '情境镜头'],
...
export const LAB_STRUCTURES: LabStructure[] = [
  {
    id: 'A',
    code: 'STR-A',
    name: '情感转化型 · 母亲节原版',
...
export const LAB_DIFF_ROWS: string[] = [
  '段落顺序',
  '节奏',
  '包装',
  '预测点击',
...
export interface LibraryVideo {
  ...
  color: string;
  segments: LibSegment[];
  packaging: { captions: string; cover: string };
  tags: string[];
}
...
export const NL_PROMPTS: string[] = [
  '把商品信息提前到第 3 秒',
  '开头更抓人,加大字反问',
  '减少字幕,节奏更快',
  '保留情感铺垫但压到 4 秒内',
...
export interface HistoryRecord {
  ...
  version: string;
  duration: string;
  status: 'done' | 'draft' | 'rendering';
  stats: { ctr: string; finish: string; convert: string };
  color: string;
  segments: RoleKey[];
  product: string;
}

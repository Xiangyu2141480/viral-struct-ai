...
export async function analyzeVideoFile(input: AnalyzeVideoFileInput): Promise<VideoAnalysis> {
  try {
    ...
    const transcript = buildTranscript(input.manualTranscript, metadata.duration);
...
function buildTranscript(manualTranscript: string | undefined, duration: number): TranscriptSegment[] {
  const normalized = manualTranscript?.trim();
  if (!normalized) {
    return [];
  }
  ...
  return chunks.map((text, index) => ({
    start: round((duration * index) / chunks.length),
    end: round(index === chunks.length - 1 ? duration : (duration * (index + 1)) / chunks.length),
    text
  }));
}
